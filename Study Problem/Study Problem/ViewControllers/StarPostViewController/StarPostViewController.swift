...
class StarPostViewController: UIViewController, UIGestureRecognizerDelegate, DZNEmptyDataSetSource, DZNEmptyDataSetDelegate {
   ...
   //  reload()
    ...
    func reload(){
        ref.child("user").child((FIRAuth.auth()?.currentUser!.uid)!).child("stars").observeSingleEvent(of: .value, with: { snapshot in
            self.posts = []
            if let snapshots = snapshot.children.allObjects as? [FIRDataSnapshot] {
                for snap in snapshots {
                    if var postDictionary = snap.value as? Dictionary<String, AnyObject> {
                        ...
                        self.ref.child("post").child(postDictionary["userstars"] as! String!).observeSingleEvent(of:.value, with: { snapshot in
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if (segue.identifier == "viewStarPost") {
            ...
            let viewcontroller: ViewpostViewController = segue.destination as! ViewpostViewController
