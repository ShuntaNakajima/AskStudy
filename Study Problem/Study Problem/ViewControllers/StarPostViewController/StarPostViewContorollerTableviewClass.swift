    ...
    func cellLongPressed(recognizer: UILongPressGestureRecognizer) {
        ...
        if indexPath == nil {
            ...
            if !longState {
                ...
                let recentUesrsQuery = reference.child("user").child((FIRAuth.auth()?.currentUser!.uid)!).child("stars").queryOrdered(byChild: "userstars").queryEqual(toValue: self.posts[indexPath!.row]["key"] as! String!)
                recentUesrsQuery.observe(.value, with: { snapshot in
                    ...
                    if mykey == ""{
                        ...
                        if self.longState {
                            ...
                            self.ref.child("user/\((FIRAuth.auth()?.currentUser!.uid)!)/stars/").child(mykey).child("userstars").removeValue()
